...
import NF_LOGO from '../../images/logohublo.png'
import ClothingDetail from '../Data/detail';


function QuickHublot({ item }) {
  ...
  const maleCardList = products.filter((maleclothesItem) => maleclothesItem.brand === 'Hublot').slice(0 ,8).map((maleclothesItem) => (
    ...
    >
      <Card style={{ width: '268px', marginBottom: '5px', border: '0.1px solid white'  }}>
        <Card.Img
          ...
          style={{ width: '267px', height: '350px',padding: ''}}
  ...
  return (
    <div>
        <a  href='hublot'>  <img   style={{ backgroundColor: "" ,padding:'', border:'solid 2px ',width:'50px',height:' 50x',color:'grey'}} src={NF_LOGO}></img></a> 
        {selectedItem && <ClothingDetail clothingItem={selectedItem} />}
    </div>
  );
}

export default QuickHublot;
