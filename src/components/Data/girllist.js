...
function Girllist({ item }) {
  const toggleDetail = (clothingItem) => {
    if (clothingItem === selectedItem) {
  ...
  const maleCardList = filteredProducts.filter((maleclothesItem) => maleclothesItem.gender === 'G').map((maleclothesItem) => (
    <Col
  ...
  return (
    <div  className='container'>
        <Search/>
        <hr/>
 <h6> &nbsp; Girl <i style={{color:'pink'}} class="fa fa-gear fa-spin" ></i> </h6>
        <Filter applyFilters={applyFilters} />
        <hr/>

        {selectedItem && (
        <>
          <button style={{float:'right'}} className="btn btn-outline-dark close-button float-right " onClick={() => toggleDetail(selectedItem)}>
            &times;
          </button>
